...
A template with `debug` set to `true` (default):
	...
	function anonymous(model,html,__vopts,vash) {
		try {
			...
			html.reportError( e, html.vl, html.vc, "<p></p>!@html.raw('L')@html.raw('B')!" );
		}
	}
...
`vash(1)` has many options:
    ...
    -t, --target-namespace @echo('<namespace>')  Assign template to a @echo('<namespace>'). Recommended is `vash.helpers.tplcache` for view engine compatibility
    -p, --property-name [name]          Assign template to property named [name]. Defaults to filename, and requires --target-namespace.
    -f, --file @echo('<file>')                   Compile the template in @echo('<file>')
    -j, --json @echo('<json>')                   Pass options to the Vash compiler. See docs for valid options.
    -o, --out @echo('<path>')                    Write template into @echo('<path>') directory
    ...
    --helpers @echo('<file>')                    Execute these compiled helpers
...
--target-namespace @lvl2('vash1--target-namespace')
---------------------------------------------------
...
--property-name @lvl2('vash1--property-name')
-----------------------------------------------
...
@html.mdFnList(function(){

	@fn('razor-ms', function(){
